...
public abstract class MarcHandler {
	...
	{       
        if (source.equals("FILE") || source.equals("STDIN"))
        {
        	...
        	logger.debug("inputTypeXML= " + inputTypeXML);
        	logger.debug("inputTypeJSON = " + inputTypeJSON);
        	logger.debug("permissiveReader = " + permissiveReader);
            if (inputTypeXML)
            {
	...
	public void loadIndexer(String indexerName, String indexerProps) 
	{
	    ...
	    try
	    {
	        ...
	        for(String searchPath : propertySearchPath){
	        	System.out.println("propertySearchPath=" + searchPath);
	        }
	        System.out.println("indexerProps=" + indexerProps);
